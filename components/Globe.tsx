...
// Fix: The pinLocation prop type is changed from Location to Tournament to resolve a logical error
// where a tournament ID was being accessed from a Location object.
...
// Fix: A type for D3.js simulation nodes is defined to include the necessary position properties (x, y)
// that are added by the simulation, resolving TypeScript errors about missing properties.
...
const Globe: React.FC<GlobeProps> = ({ pinLocation, selectedMonth, tournaments, onTournamentSelect }) => {
  ...
  useEffect(() => {
    ...
    if (!projection) {
        ...
        svg.append('path').datum({ type: 'Sphere' }).attr('class', 'sphere').attr('fill', '#1F2937').attr('stroke', '#4B5563')
            .on('click', () => {
                onTournamentSelect(null);
                setActiveTournament(null);
            });
        svg.append('path').datum(d3.geoGraticule10()).attr('class', 'graticule').attr('fill', 'none').attr('stroke', '#374151').attr('stroke-width', 0.5);

        d3.json('https://unpkg.com/world-atlas@2/countries-110m.json').then((world: any) => {
            svg.insert('path', '.graticule').datum(topojson.feature(world, world.objects.countries)).attr('class', 'land').attr('fill', '#374151').attr('stroke', '#1F2937');
        });
        ...
        const zoom = d3.zoom<SVGSVGElement, unknown>().scaleExtent([1, 10])
          .on('start', (event) => {
          ...
          .on('zoom', (event) => {
            ...
            // If scale hasn't changed, it's a pan gesture (drag).
            if (currentScale === previousTransform.k) {
              ...
              // It's a zoom gesture. We need to adjust the rotation to keep the point under the cursor fixed.
              const pointer = d3.pointer(event, svg.node());
              const geoPointBeforeZoom = projection.invert(pointer);
              
              projection.scale(initialScaleRef.current * currentScale); // Update scale first
              
              const screenPointAfterZoom = projection(geoPointBeforeZoom);

              const dx = pointer[0] - screenPointAfterZoom[0];
              const dy = pointer[1] - screenPointAfterZoom[1];
    ...
    const drawPins = () => {
        ...
        const visibleTournaments = tournamentsToShow.filter(t => {
            ...
            return d3.geoDistance(coords, [-finalProjection.rotate()[0], -finalProjection.rotate()[1]]) <= Math.PI / 2;
        });

        const nodes: TournamentNode[] = visibleTournaments.map(t => {
            const [x, y] = finalProjection( [t.location.lon, t.location.lat]) as [number, number];
            return { ...t, fx: x, fy: y };
        });
        ...
        const pinGroups = svg.selectAll('.pin-group').data(nodes, d => (d as any).id).join('g')
        ...
        pinGroups.append('circle').attr('r', 5).attr('fill', d => getPointCategoryColor(d.points)).attr('stroke', '#111827').attr('stroke-width', 1);
    ...
    // Fix: Updated logic to handle pinLocation as a Tournament object and access its location property.
    if (pinLocation) {
